//boiler plate code gotten from https://github.com/Overv/VulkanTutorial
...
public:
	ParticleSystem(uint32_t type) : TYPE(type) {}
...
private:
	...
	#define PARTICLE_TYPE_SPARKS 1

	float minFireVel = 0.5f;
	float maxFireVel = 10.0f;

	float minSparkVel = 0.0f;
	float maxSparkVel = 20.0f;
	...
	VDeleter<VkBuffer> buffer{ device, vkDestroyBuffer };
	VDeleter<VkDeviceMemory> memory{ device, vkFreeMemory };
	...
	VDeleter<VkDeviceMemory> uniformMemory{ device, vkFreeMemory };
	...
	void initWindow() {
		...
		window = glfwCreateWindow(WIDTH, HEIGHT, "Particle_System", nullptr, nullptr);
	}
	...
	void prepareParticles() {
		...
		for (auto& particle : particleBuffer)
		{
			...
			case PARTICLE_TYPE_FLAME:
				initParticle(&particle, glm::vec3(0.0, 1.5, 0.0));
				break;
			case PARTICLE_TYPE_SPARKS:
				initParticle(&particle, glm::vec3(0.01, 0.01, 0.01));
				break;
			}
		}

		buffersize = particleBuffer.size() * sizeof(Particle);

		createBuffer(buffersize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, buffer, memory);

		if (vkMapMemory(device, memory, 0, buffersize, 0, &mappedMemory) != VK_SUCCESS) {
			throw std::runtime_error("failed to map memory in prepare particles!");
		}
	...
	void initParticle(Particle *particle, glm::vec3 emitterPos) {
		...
		switch (TYPE) {
		case PARTICLE_TYPE_FLAME:
			particle->vel = glm::vec3(0.0, minFireVel + rnd(maxFireVel - minFireVel), 0.0);
			particle->alpha = rnd(0.75f);
			particle->size = rnd(1.5f);
			particle->color = glm::vec4(0.25 + rnd(0.25));
			particle->type = PARTICLE_TYPE_FLAME;
			particle->rotation = rnd(2.0f * float(M_PI));
			particle->rotationSpeed = rnd(2.0f) - rnd(2.0f);
			particle->x_time = 0;
			particle->x_dir = rnd_sign();
			...
			break;
		case PARTICLE_TYPE_SPARKS:
			particle->vel = glm::vec3(rnd_sign() * rnd(maxSparkVel - minSparkVel), rnd_sign() * rnd(maxSparkVel - minSparkVel), rnd_sign() * rnd(maxSparkVel - minSparkVel));
			particle->alpha = rnd(0.75f);
			particle->size = rnd(0.25f);
			particle->color = glm::vec4(1.0);
			particle->type = PARTICLE_TYPE_SPARKS;
			particle->rotation = rnd(2.0f * float(M_PI));
			particle->rotationSpeed = rnd(2.0f) - rnd(2.0f);
			particle->x_time = 0;
			particle->x_dir = 0;
	...
	void updateParticles() {
		...
		for (auto& particle : particleBuffer)
		{
			...
			case PARTICLE_TYPE_FLAME:
				...
				if (particle.x_time != 200) {
					particle.pos.x += particle.x_dir * .0005f;
					++particle.x_time;
				}
				else {
					particle.x_dir = rnd_sign();
				...
				particle.size -= particleTimer * 4.0f;
			...
			case PARTICLE_TYPE_SPARKS:
				...
				particle.vel.y += particleTimer * 9.8f * 5.0f;
				particle.pos.y += particleTimer * particle.vel.y;
				particle.pos.x += particleTimer * particle.vel.x;
				particle.pos.z += particleTimer * particle.vel.z;
				particle.alpha += particleTimer * 2.5f;
				particle.size -= particleTimer * 1.0f;
				particle.color -= particleTimer * 0.05f;
				break;
			}
		...
		memcpy(mappedMemory, particleBuffer.data(), size);
	}
	...
	void createInstance() {
		...
		appInfo.pApplicationName = "Particle_System";
	...
	void createGraphicsPipeline() {
		...
		vertexInputBinding.binding = VERTEX_BUFFER_BIND_ID;
		vertexInputBinding.stride = sizeof(Particle);
		vertexInputBinding.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

		std::array<VkVertexInputAttributeDescription, 6> vertexInputAttributes = {};
		vertexInputAttributes[0].binding = VERTEX_BUFFER_BIND_ID;
		vertexInputAttributes[0].location = 0;
		vertexInputAttributes[0].format = VK_FORMAT_R32G32B32A32_SFLOAT;
		vertexInputAttributes[0].offset = offsetof(Particle, pos);

		vertexInputAttributes[1].binding = VERTEX_BUFFER_BIND_ID;
		vertexInputAttributes[1].location = 1;
		vertexInputAttributes[1].format = VK_FORMAT_R32G32B32A32_SFLOAT;
		vertexInputAttributes[1].offset = offsetof(Particle, color);

		vertexInputAttributes[2].binding = VERTEX_BUFFER_BIND_ID;
		vertexInputAttributes[2].location = 2;
		vertexInputAttributes[2].format = VK_FORMAT_R32_SFLOAT;
		vertexInputAttributes[2].offset = offsetof(Particle, alpha);

		vertexInputAttributes[3].binding = VERTEX_BUFFER_BIND_ID;
		vertexInputAttributes[3].location = 3;
		vertexInputAttributes[3].format = VK_FORMAT_R32_SFLOAT;
		vertexInputAttributes[3].offset = offsetof(Particle, size);

		vertexInputAttributes[4].binding = VERTEX_BUFFER_BIND_ID;
		vertexInputAttributes[4].location = 4;
		vertexInputAttributes[4].format = VK_FORMAT_R32_SFLOAT;
		vertexInputAttributes[4].offset = offsetof(Particle, rotation);

		vertexInputAttributes[5].binding = VERTEX_BUFFER_BIND_ID;
		vertexInputAttributes[5].location = 5;
		vertexInputAttributes[5].format = VK_FORMAT_R32_SINT;
		vertexInputAttributes[5].offset = offsetof(Particle, type);
		...
		colorBlendAttachment.dstColorBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_COLOR; // Optional
		...
		if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, pipeline.replace()) != VK_SUCCESS) {
	...
	void createUniformBuffer() {
		...
		createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uniformBuffer, uniformMemory);
	}

	void updateUniformBuffer() {
		UBOVS uboVS = {};
		uboVS.model = glm::mat4(1.0f);
		uboVS.view = glm::lookAt(glm::vec3(0.0f, 0.0f, 10.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f));
		uboVS.projection = glm::perspective(glm::radians(45.0f), swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10.0f);
		uboVS.projection[1][1] *= -1;
		uboVS.viewportDim = glm::vec2((float)WIDTH, (float)HEIGHT);

		void* data;
		vkMapMemory(device, uniformMemory, 0, sizeof(uboVS), 0, &data);
...
int main() {
	uint32_t t;
	std::string type;
	std::cout << "Particle system choices are fire and sparks.\nPlease enter a particle system: ";
	...
	if (type == "fire")
		...
		std::cout << "No valid type was given, will now default to fire" << std::endl;
		t = 0;
	}

	ParticleSystem app(t);
